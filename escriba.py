...
# ─── Expressões Regulares Compiladas (Otimização) ─────────────────────────
HTML_TAG_RE = re.compile(r"<[^>]+>")
WEBVTT_HEADER_RE = re.compile(r"^WEBVTT.*?\n\n", flags=re.DOTALL)
CAPITALIZE_SENTENCE_RE = re.compile(r'(^|[.!?]\s+)(\w)')
try:
...
def _flush_paragraph(lines: list[str], para_ts: str, out: list[str]) -> None:
    ...
    if text:
        text = CAPITALIZE_SENTENCE_RE.sub(lambda m: m.group(0)[:-1] + m.group(0)[-1].upper(), text)
...
def convert_vtt_to_srt(vtt_path: Path) -> Path:
    ...
    if content.startswith("WEBVTT"):
        content = WEBVTT_HEADER_RE.sub("", content)
