...
Este script segue as regras de Clean Code Naming e PEP 8: variáveis
...
# ─── Interrupção Global e UTF-8 para Windows ────────────────────────────────
...
# (requests, urllib3, yt-dlp) sejam as do projeto e não as globais do sistema.
_script_dir: Path = Path(__file__).parent.resolve()
_venv_bin: Path = _script_dir / ".venv" / ("Scripts" if os.name == "nt" else "bin")
_venv_python: Path = _venv_bin / ("python.exe" if os.name == "nt" else "python3")
...
# Suprime avisos de dependências (urllib3/requests)
...
from rules import (
    clean_ekklezia_terms,
    fix_sentence_capitalization,
    restore_punctuation_heuristics,
)
from utils import (
    BCYAN,
    BGREEN,
...
class SessionConfig:
    ...
    channel_url: str
    discovered_uploader_id: str | None = None
    provider: str = "youtube"  # 'youtube' ou 'vimeo'
    disk_files_cache: dict[str, list[str]] | None = None  # Cache: { "vid_id": ["file1.srt", "file1.md"] }
...
# ─── Extração de Metadados e Origem ──────────────────────────────────────────
def _extract_video_meta_cli(
    url_str: str,
    cmd_list: list[str],
    cookie_args_list: list[str],
) -> tuple[str | None, str | None]:
    """Executa comando yt-dlp para extrair metadados de vídeo único."""
    meta_cmd_list: list[str] = (
        cmd_list + cookie_args_list + ["--dump-json", "--skip-download", url_str]
    )
    try:
        proc_res_obj = subprocess.run(
            meta_cmd_list,
            capture_output=True,
            text=True,
            encoding="utf-8",
        ...
        if proc_res_obj.stdout:
            meta_dict: dict[str, Any] = json.loads(proc_res_obj.stdout)
            return (
                meta_dict.get("uploader_id") or meta_dict.get("uploader"),
                meta_dict.get("channel_id"),
            )
...
def _extract_playlist_meta_cli(
    url_str: str,
    cmd_list: list[str],
    cookie_args_list: list[str],
    ...
    meta_cmd_list = (
        cmd_list
        + cookie_args_list
        + ["--dump-json", "--flat-playlist", "--playlist-end", "1", "--ignore-errors", url_str]
    )
    try:
        ...
        if proc_res.stdout:
            ...
            return (
                meta_dict.get("uploader_id") or meta_dict.get("uploader"),
                meta_dict.get("channel_id"),
                meta_dict.get("uploader"),
            )
    except (json.JSONDecodeError, subprocess.SubprocessError, OSError, IndexError):
        pass
    return None, None, None


def _identify_playlist_source(
    url_str: str,
    history_dict: dict[str, Any],
    cmd_list: list[str],
    cookie_args_list: list[str],
    ...
    up_id_str, chan_id_str, _ = _extract_playlist_meta_cli(url_str, cmd_list, cookie_args_list)
    if up_id_str:
        print_ok(f"Origem identificada: {BOLD}@{up_id_str.lstrip('@')}{RESET}")
...
def identify_source_type(
    ...
    if (
        ...
        or (
            provider == "vimeo"
            and re.search(r"vimeo\.com/\d+", url_str)
            and not any(x in url_str for x in ["/showcase/", "/channels/"])
        )
    ):
...
def consolidar_dados_legados(cwd_path: Path, target_json_path: Path) -> None:
    ...
    for lj_path in legacy_list:
        if lj_path.resolve() == target_json_path.resolve():
            continue
...
def _import_reverse_history(
    state_dict: dict[str, dict[str, Any]],
    history_dict: dict[str, Any],
    chan_id_str: str | None,
    up_id_str: str | None,
    name_str: str | None,
    tag_str: str | None,
) -> int:
    ...
    for vid_id_str, hist_dict in history_dict.items():
        ...
        is_same_bool: bool = bool(
            (chan_id_str and hist_dict.get("channel_id") == chan_id_str)
            or (up_id_str and hist_dict.get("uploader_id") == up_id_str)
            or (name_str and name_str.lower() in str(hist_dict.get("uploader", "")).lower())
        )

        if is_same_bool:
            new_entry_dict: dict[str, Any] = hist_dict.copy()
...
def _load_existing_state_map(json_path: Path) -> dict[str, dict[str, Any]]:
...
def _load_initial_metadata(
    cwd_path: Path,
    url_str: str,
    cmd_list: list[str],
    cookies_list: list[str],
) -> tuple[Path, dict[str, Any], str, str, str | None, str | None]:
...
def load_or_create_channel_state(
    ...
    cookies_list: list[str],
    url_str: str,
    only_peek_lang_bool: bool = False,
) -> tuple[Path | None, list[dict[str, Any]], str | None, int]:
    """Carrega ou cria o banco de dados JSON do canal."""
    json_path, hist_dict, name_str, ident_str, chan_id_str, up_id_str = _load_initial_metadata(
        cwd_path, url_str, cmd_list, cookies_list
    )
    lang_cached_str = _read_persisted_lang(json_path)
    if only_peek_lang_bool:
        return json_path, [], lang_cached_str, 0
    ...
    yt_list = provider_mod.generate_fast_list_json(
        cmd_list, cookies_list, url_str, history_dict=hist_dict, stop_at_ids=stop_ids
    )
    if not yt_list and not state_dict:
        return None, [], lang_cached_str, 0

    tag_str = f"@{name_str}" if name_str and name_str != "canal" else url_str
    _perform_state_sync(
        state_dict, yt_list, ident_str, tag_str, chan_id_str, up_id_str, name_str, cwd_path, url_str, history_dict=hist_dict
    )
...
def _perform_state_sync(
    ...
    new_v_int, imp_v_int = sync_video_records(
        ...
        yt_list,
        history_dict,
        (ident_str if "list=" in url_str else None),
        tag_str,
        chan_id_str,
        up_id_str,
        name_str,
    ...
    if ghosts_int:
        print_ok(f"Limpeza Automática: Removidos {BOLD}{ghosts_int}{RESET} IDs fantasmas.")
    if new_v_int > 0:
        print_ok(f"Descobertos {BOLD}{new_v_int}{RESET} novos vídeos.")
    if imp_v_int > 0:
        print_ok(f"Importados {BOLD}{imp_v_int}{RESET} vídeos do histórico local.")
...
# ─── Cache de dependências de ML (Lazy Loading) ──────────────────────────────
@functools.lru_cache(maxsize=1)
def _load_ml_deps() -> tuple[Any, Any, Any, Any, Any, Any] | None:
    """Importa dependências de ML (pysrt, sklearn, nltk) sob demanda."""
    try:
        import nltk
...
def _get_lang_resources(lang_code_str: str) -> tuple[str, set[str]]:
    ...
    return lang_map.get(prefix_str, ("portuguese", ORAL_MARKERS_PT))


@functools.lru_cache(maxsize=8)
def get_merged_stopwords(lang_code_str: str) -> frozenset[str]:
...
def _strip_rollup(text_str: str, prev_text_str: str, overlap_ratio_float: float = 0.5) -> str:
    ...
    prev_tokens_list = prev_text_str.split()
    cur_tokens_list = text_str.split()
...
def _seg_keywords(
    seg_wins_list: list[dict[str, Any]],
    tfidf_vec_obj: Any,
    tfidf_mat_obj: Any,
    win_indices_list: list[int],
    oral_stopwords_set: frozenset[str],
    top_n_int: int = 3,
) -> str:
    ...
    for i_int in top_indices_list:
        ...
        if len(keywords_list) >= top_n_int:
            break
...
def create_adaptive_windows(
    subs_list: list[Any],
    window_size_s_int: int,
) -> tuple[list[dict[str, Any]], dict[int, str]]:
    """Agrupa legendas em janelas adaptativas aplicando restauração local de pontuação."""
    windows_list: list[dict[str, Any]] = []
    current_window_subs_list: list[Any] = []
    ...
    for i_int, sub_obj in enumerate(subs_list):
        ...
        if clean_text_str:
            ...
            if not is_last_bool:
                ...
                cur_end = sub_obj.end
                pause_s_float = max(0.0, float(_calc_total_seconds(next_start) - _calc_total_seconds(cur_end)))
        ...
        if elapsed_window_s > window_size_s_int and current_window_subs_list:
            ...
            if w_text_str:
                windows_list.append({
                    "text": w_text_str,
                    "timestamp": str(current_window_subs_list[0].start).split(",")[0],
                    "subs": current_window_subs_list,
                })
            current_window_subs_list, start_time_obj = [], sub_obj.start

    if current_window_subs_list:
        w_text_str = " ".join(clean_texts.get(id(s), "") for s in current_window_subs_list)
        if w_text_str:
            windows_list.append({
                "text": w_text_str,
                "timestamp": str(current_window_subs_list[0].start).split(",")[0],
...
def detect_topic_breaks(
    tfidf_matrix: Any,
    adapt_thresh: float,
    cosine_similarity_func: Callable,
    min_segs: int = 3,
) -> set[int]:
...
def assemble_segments(
    windows_list: list[dict[str, Any]],
    topic_breaks_set: set[int],
) -> list[tuple[str, int, list[dict[str, Any]]]]:
    """Agrupa janelas em segmentos de tópicos."""
    segments_list: list[tuple[str, int, list[dict[str, Any]]]] = []
    current_seg_wins_list: list[dict[str, Any]] = []
...
def yaml_safe(template: Any) -> str:
...
def _init_md_processing(srt_path: Path, indentation_prefix: str) -> tuple[Any, Any] | None:
...
def _process_sub_into_para(
    sub: Any,
    para_start_time: Any,
    para_lines_list: list[str],
    md_lines: list[str],
    sentence_end_re: re.Pattern,
...
def _generate_md_body_sections(
    md_lines: list[str],
    segs_list: list[tuple[str, int, list[dict[str, Any]]]],
    vec: Any,
    tfidf_mat: Any,
    stops: frozenset[str],
    clean_texts: dict[int, str] | None = None,
...
def srt_to_md(
    ...
    video_id_str: str,
    video_title_str: str,
    video_date_str: str = "Desconhecida",
    threshold_float: float = 0.3,
    indentation_prefix_str: str = "  ",
) -> Path | None:
    """Converte legendas .srt em arquivos .md estruturados com segmentação semântica por tópicos."""
        ...
        if valid_fallbacks:
            for f_str in valid_fallbacks:
                ...
                if suffix.endswith(".srt"):
                    lang_part = suffix[:-4]
                    new_name = (
                        f"{channel_dir_name}-{video_id}-{lang_part}.srt"
                        if lang_part
                        else f"{channel_dir_name}-{video_id}.srt"
                    )
                else:
...
def cleanup_subtitles(
    ...
    channel_dir_name: str,
    video_id_str: str,
    video_title_str: str = "Vídeo Sem Título",
    convert_srt_to_md_bool: bool = False,
    keep_srt_bool: bool = False,
    indent_str: str = "  ",
...
def _rename_to_standard_format(
    target_path: Path,
    channel_dir_name: str,
    video_id_str: str,
) -> Path:
...
def _extract_meta_to_dict(meta_dict: dict[str, Any], video_dict: dict[str, Any]) -> None:
...
def _add_utility_args(parser_obj: argparse.ArgumentParser) -> None:
    ...
    parser_obj.add_argument("--consolidar", "-j", "--juntar", action="store_true", help="Gera ou atualiza os volumes unificados do NotebookLM. Alias: -j / --juntar")
    ...
    parser_obj.add_argument("--threshold", type=float, default=DEFAULT_THRESHOLD, metavar="FLOAT",
                            help=f"Sensibilidade de detecção de tópicos (0.0-1.0). Padrão: {DEFAULT_THRESHOLD}")
...
def _print_session_info(cli_args: argparse.Namespace, latest_json_path: Path | None) -> None:
    ...
    if cli_args.date:
        mode_label_str += f"  ·  a partir de {cli_args.date}"
    if cli_args.fast:
        mode_label_str += "  ·  rápido"

    print_info(f"Canal alvo: {BOLD}{cli_args.canal}{RESET}")
    print_info(f"Modo: {mode_label_str}")
    if latest_json_path:
        print_info(f"Auto-detecção via {latest_json_path.name}")
...
def _warm_up_cookies(session_config: SessionConfig, cookie_args_list: list[str]) -> list[str]:
    ...
    if not cookies_txt_path.is_file():
        ...
        try:
            ...
            print_warn("Warm-up timeout exceeded.")
    ...
    if cookies_txt_path.is_file():
        ...
        return configure_cookies(
            session_config.cwd_path, session_config.script_dir_path, False, silent_bool=True, browser_str=session_config.browser_name
        )
...
def init_auth_and_language(
    ...
    language_arg_str: str,
    force_refresh_flag: bool,
) -> tuple[list[str], str]:
    """Etapa 2: configura autenticação e idioma."""
    print_section("Autenticação")
    cookies_list = configure_cookies(
        session_config.cwd_path, session_config.script_dir_path, force_refresh_flag, browser_str=session_config.browser_name
    )
...
def _detect_and_report_language(
    session_config: SessionConfig,
    cookies_list: list[str],
    user_lang_str: str,
) -> str:
    ...
    if not cached_lang_str:
        _, _, cached_lang_str, _ = load_or_create_channel_state(
            session_config.cwd_path,
            session_config.yt_dlp_cmd_list,
            cookies_list,
            session_config.channel_url,
...
def _filter_by_single_video(state_list: list[dict[str, Any]], video_id_str: str) -> list[dict[str, Any]]:
...
def _filter_by_handle(
    ...
    def normalize(h: Any) -> str:
        return str(h).lower().replace(".official", "").replace("-official", "")

    target_norm = normalize(handle_str)

    filtered_list = [v for v in working_list if normalize(v.get("source_channel", "")) == target_norm]
...
def _filter_working_videos(
    ...
    input_type_str: str,
    single_video_id_str: str | None,
    channel_filter_str: str | None,
    is_first_channel_bool: bool,
...
def _check_disk_files(
    ...
    video_dict: dict[str, Any],
    cli_args: argparse.Namespace,
    pending_md_list: list[tuple[Path, str, str, str]],
    prefix_str: str,
    idx_int: int = 0,
    total_int: int = 0,
) -> bool:
    ...
    if session_config.disk_files_cache is not None:
        ...
        srt_list: list[str] = []
        md_list: list[str] = []
        # BOLT OPTIMIZATION: Avoid redundant list comprehensions and Path instantiation overhead.
        # Single pass partitioning using string concatenation.
    ...
    _handle_found_disk_files(
        video_id_str, video_dict, srt_list, md_list, cli_args, pending_md_list, prefix_str, idx_int, total_int
    )
...
def _handle_found_disk_files(
    ...
    if srt_list and not md_list and args.md:
        ...
        pending.append((
            Path(srt_list[0]),
            vid_id,
            v_dict.get("title", "Sem Título"),
            v_dict.get("publish_date", "N/A"),
        ))
...
def _prepare_working_state(
    conf_obj: SessionConfig,
    cookies_list: list[str],
    lang_str: str,
    cli_args_ns: argparse.Namespace,
    channel_filter_str: str | None,
    is_first_channel_bool: bool,
) -> tuple[Path | None, list[dict[str, Any]], list[dict[str, Any]], int]:
...
def process_videos(
    ...
    lang_str: str,
    cli_args_ns: argparse.Namespace,
    channel_filter_str: str | None = None,
    is_first_channel_bool: bool = True,
    ...
    try:
        ...
        if not working_list:
            if is_first_channel_bool:
                print_ok("Nenhum vídeo novo para processar.")
        ...
        no_subtitle_ids = {v["video_id"] for v in working_list if v.get("has_no_subtitle") and not retry_nosub}
        disk_ids = set(disk_cache)
        all_skip_ids = already_downloaded_ids | no_subtitle_ids | disk_ids

        pending_md_pre: list[tuple[Path, str, str, str]] = []
            ...
            if vid_id in disk_ids and vid_id not in already_downloaded_ids:
                ...
                if has_srt and not has_md and cli_args_ns.md:
                    ...
                    pending_md_pre.append((
                        conf_obj.cwd_path / srt_f,
                        vid_id,
                        v.get("title", "Sem Título"),
                        v.get("publish_date", "N/A"),
                    ))
...
def _run_video_download_loop(
    ...
    to_download_list: list[dict[str, Any]],
    pre_skip_count: int = 0,
    total_count: int = 0,
) -> tuple[tuple[int, int, int, int], bool, list[tuple[Path, str, str, str]]]:
    """Executa o loop de download sobre vídeos pré-filtrados pelo particionamento."""
    stats_list = [0, pre_skip_count, 0]  # [dl, skip, err]
    pending_md: list[tuple[Path, str, str, str]] = []
    dirty: list[int] = [0]
    interrupted = False
...
def _check_auto_save(
    ...
    full_list: list[dict[str, Any]],
    lang_str: str,
    session_config_obj: SessionConfig,
    dirty_list: list[int],
) -> None:
    """Salva o estado automaticamente a cada 5 alterações."""
    if json_path and dirty_list[0] >= 5:
...
def _handle_post_download(
    ...
    ok_bool, srt_path = cleanup_subtitles(
        session_config.cwd_path,
        session_config.channel_dir_name,
        video_dict["video_id"],
        video_dict.get("title", "Sem Título"),
        convert_srt_to_md_bool=cli_args_ns.md,
        keep_srt_bool=cli_args_ns.keep_srt,
...
def _run_deferred_md_conversion(
    pending_list: list[tuple[Path, str, str, str]],
    cli_args_ns: argparse.Namespace,
) -> bool:
    ...
    def _convert_task(item: tuple[Path, str, str, str]) -> tuple[str, Path | None]:
        ...
        if use_parallel_bool:
            ...
            for idx_int, item in enumerate(pending_list, start=1):
                ...
                if md_res_path:
                    ...
                    print_warn("falha na conversão", prefix_str)
...
def _load_video_lookup(cwd_path: Path) -> dict[str, dict[str, str]]:
...
def _run_regen_loop(
    ...
    if use_parallel_bool:
        ...
        def _regen_task(indexed_item: tuple[int, tuple[Path, str]]) -> tuple[int, str]:
...
def _print_srt_group_header(label_str: str, srt_list: list[tuple[Path, str]]) -> None:
...
def _process_srt_item(
    ...
    res_path = srt_to_md(
        ...
        vid_id_str,
        title_str,
        video_date_str=date_str,
        threshold_float=DEFAULT_THRESHOLD,
        indentation_prefix_str="      ",
    )
...
def _scan_md_files(cwd_path: Path) -> list[tuple[Path, str]]:
    ...
    scan_dirs_list: list[tuple[Path, str]] = [(cwd_path, "./")]
    for arch_dir in ["archive", "archives"]:
        if (cwd_path / arch_dir).is_dir():
            scan_dirs_list.append((cwd_path / arch_dir, f"{arch_dir}/"))

    md_files_list: list[tuple[Path, str]] = []
    for scan_dir_path, label_str in scan_dirs_list:
...
def _extract_old_metadata(content_str: str) -> dict[str, str | None]:
...
def _build_new_md_header(md_path: Path, meta_dict: dict[str, str | None]) -> str:
    """Extrai ID e gera novo cabeçalho YAML."""
    vid_id_str = "unknown"
    url_val = meta_dict.get("url") or ""
    if "vimeo.com/" in url_val:
        vimeo_id_match = re.search(r"vimeo\.com/(\d+)", url_val)
        if vimeo_id_match:
            vid_id_str = vimeo_id_match.group(1)
    else:
        yt_id_match = re.search(r"watch\?v=([A-Za-z0-9_-]{11})", url_val)
        if yt_id_match:
            vid_id_str = yt_id_match.group(1)

    header_list = generate_md_header(
        meta_dict.get("title") or md_path.stem,
        vid_id_str,
        meta_dict.get("date") or "Desconhecida",
        meta_dict.get("duration") or "N/A",
        "unknown",
        VERSION,
    )
...
def _validate_direct_download_target(canal_arg_any: list[str] | str | None) -> tuple[str, str]:
...
def _run_direct_video_download_flow(cli_args_ns: argparse.Namespace) -> None:
    ...
    cookies_list = configure_cookies(
        Path.cwd(),
        script_dir_path,
        cli_args_ns.refresh_cookies,
        silent_bool=False,
        browser_str=getattr(cli_args_ns, "browser", "firefox"),
    )
    _filter_cookies_if_present(Path.cwd(), provider_str)

    print_info("Iniciando download de vídeo direto em Full HD...")
...
def exibir_status_canais_json() -> None:
    """Localiza todos os arquivos de banco de dados escriba_*.json na pasta de trabalho e exibe estatísticas como JSON."""
    cwd_path = Path.cwd()
...
def _handle_new_channel_flow(
    ...
    res_tuple = process_videos(
        conf_obj, cookies_list, lang_str, cli_args_ns, channel_filter_str=canal_str, is_first_channel_bool=False
    )
    ...
    try:
        for idx_int, channel_str in enumerate(channels_list, 1):
            ...
            res_tuple = _process_channel_sync_item(
                channel_str, cli_args_ns, idx_int, is_multi_bool, global_cookies_list=global_cookies_list
            )
            if res_tuple:
...
def _accumulate_multi_stats(stats_list: list[int], res_tuple: tuple[int, int, int, int, int, bool]) -> None:
...
def _process_channel_sync_item(
    ...
    idx_int: int,
    is_multi_bool: bool,
    global_cookies_list: list[str] | None = None,
...
def _finish_session_flow(
    res_tuple: tuple[int, int, int, int, int, bool],
    cli_args_ns: argparse.Namespace,
    cwd_path: Path,
    multi: bool = False,
) -> None:
...
def _run_main_sync_flow(
    json_path: Path,
    user_canal_str: str | None,
    cli_args_ns: argparse.Namespace,
    cwd_path: Path,
) -> None:
...
def _normalize_canal_arg(cli_args: argparse.Namespace) -> None:
    ...
    if not isinstance(cli_args.canal, list) or not cli_args.canal:
        if isinstance(cli_args.canal, list):
            cli_args.canal = None
        return
...
def main() -> None:
    """Função de entrada principal da CLI."""
