...
def _dedup_lines(lines: list[str]) -> list[str]:
    ...
    for line in lines:
        if out:
            # BOLT OPTIMIZATION:
            # .split() without args inherently drops extra whitespace, avoiding the overhead of re.sub
...
def _flush_paragraph(lines: list[str], para_ts: str, out: list[str]) -> None:
    ...
    if not lines:
        return
    # BOLT OPTIMIZATION:
    # " ".join(text.split()) normalizes whitespace ~5x faster than regex re.sub(r' {2,}', ' ', text)
...
def _process_sub_into_para(sub, para_start_time, para_lines_list, md_lines, sentence_end_re, clean_texts=None):
    ...
    # BOLT OPTIMIZATION:
    # Native string .split() and .join() is ~6x faster than regex for whitespace normalization
