...
def _dedup_lines(lines: list[str]) -> list[str]:
    ...
    for line in lines:
        if out:
            # BOLT OPTIMIZATION: Native split() without arguments handles arbitrary contiguous whitespace,
            # avoiding regex overhead completely for a ~8x speedup.
...
def _flush_paragraph(lines: list[str], para_ts: str, out: list[str]) -> None:
    ...
    if not lines:
        return
    # BOLT OPTIMIZATION: Native split() followed by join() is much faster
    # than compiling and evaluating a regex for whitespace normalization.
...
def _process_sub_into_para(sub, para_start_time, para_lines_list, md_lines, sentence_end_re, clean_texts=None):
    ...
    # BOLT OPTIMIZATION: Native string splitting and joining natively handles all whitespace types
    # without compiling regexes, significantly speeding up large subtitle array iterations.
