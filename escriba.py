...
def _dedup_lines(lines: list[str]) -> list[str]:
    ...
    out = []
    # BOLT OPTIMIZATION:
    # Use native str.split() which automatically handles all whitespace normalization,
    # avoiding the overhead of re.sub(r'\s+', ' ', ...) in string processing hot paths.
    for line in lines:
        if out:
...
def _flush_paragraph(lines: list[str], para_ts: str, out: list[str]) -> None:
    ...
    if not lines:
        return
    # BOLT OPTIMIZATION:
    # Native " ".join(str.split()) is significantly faster than re.sub for spacing normalization
...
def _process_sub_into_para(sub, para_start_time, para_lines_list, md_lines, sentence_end_re, clean_texts=None):
    ...
    # BOLT OPTIMIZATION: Native " ".join(str.split()) is significantly faster than re.sub for spacing normalization
