...
# BOLT OPTIMIZATION: Pre-compile regular expressions globally to avoid cache lookup and recompilation overhead in hot paths
...
_TITLE_CLEANUP_PATTERN: Pattern = re.compile(r'[\*\_]')
_YAML_FRONTMATTER_PATTERN: Pattern = re.compile(r"^---\n.*?\n---\n?", flags=re.DOTALL)
...
def clean_srt_content(subtitle_content_str: str) -> str:
    ...
    for block_list_list in subtitle_blocks_list:
        cleaned_lines_list.extend(block_list_list)
...
def _get_deduplication_start_index(prev_list_list: List[str], curr_list_list: List[str]) -> int:
    """
    Compara o bloco anterior com o atual para saber onde começa o texto inédito.
    
    EXPLICAÇÃO PARA JUNIORES:
    Imagine que o bloco anterior (prev_list_list) terminou com "Olá".
    Se o bloco atual (curr_list_list) começa com "Olá", "Mundo", nós já lemos o "Olá".
...
def _extract_metadata_from_content(content_str: str) -> Dict[str, str]:
    """
    ...
    if yaml_match_obj:
        ...
        for key_str, pattern_str in fields_dict.items():
            ...
            if field_match_obj:
                ...
                if val_str:
                    if key_str == "date":
                        meta_dict["date"] = format_date(val_str)
                    else:
                        meta_dict[key_str] = val_str
    ...
    if "title" not in meta_dict:
        ...
        if h1_match_obj:
            ...
            title_candidate = _TITLE_CLEANUP_PATTERN.sub('', title_candidate)
            if title_candidate:
                meta_dict["title"] = title_candidate
            
    if "date" not in meta_dict:
        # Busca variações de > **Data:** YYYY-MM-DD ou **Data**: 20260316
        ...
        for pattern in date_patterns:
            ...
            if date_match:
                ...
                if extracted_date:
                    meta_dict["date"] = format_date(extracted_date)
                    break # Para na primeira que achar
...
def get_metadata(file_path_str: str) -> Dict[str, str]:
    ...
    2. Conteúdo do arquivo (Frontmatter): Para arquivos .md que já possuem metadados internos.
    3. Nome do arquivo: Se o .json sumir, tentamos extrair o ID do nome.
    """
    ...
    # Fallback 2: Tenta ler metadados de dentro do arquivo (especialmente para .md)
    ...
    # Fallback 3: Tenta adivinhar o ID pelo nome do arquivo.
    id_from_name_str: str = _extract_id_from_filename(file_path_str)
    
    return {
        "date": internal_meta.get("date") or "Desconhecida",
        "title": internal_meta.get("title") or "Sem Título",
        "id": internal_meta.get("id") or id_from_name_str
    }
...
def _generate_json_candidates(file_path_str: str) -> List[str]:
    ...
    for sep_char_str in ['.', '-']:
        while sep_char_str in temp_str:
            temp_str = temp_str.rsplit(sep_char_str, 1)[0]
            if temp_str:
...
def _load_metadata_from_json(json_path_str: str) -> Optional[Dict[str, str]]:
    ...
    try:
        with open(json_path_str, 'r', encoding='utf-8') as file_descriptor_obj:
            data_dict_dict: Dict = json.load(file_descriptor_obj)
            raw_date_any = data_dict_dict.get('upload_date') or data_dict_dict.get('publish_date') or data_dict_dict.get('date')
            return {
                "date": format_date(raw_date_any),
                "title": data_dict_dict.get('title', 'Sem Título'),
                "id": data_dict_dict.get('id', 'Sem ID')
            }
    except Exception:
        return None
...
def _extract_id_from_filename(file_path_str: str) -> str:
    ...
    """
    return extract_video_id(file_path_str)
...
def _extract_md_transcription(md_content_str: str) -> str:
    ...
    yaml_stripped_str: str = _YAML_FRONTMATTER_PATTERN.sub("", content_str, count=1).strip()
...
def _migrate_legacy_state(channel_dir_path_str: str, state_file_path_str: str) -> None:
    ...
    try:
        ...
        print_warn(f"Falha ao migrar estado legado: {error_obj}", "  ")
...
def _perform_legacy_migration(legacy_path_str: str, state_file_path_str: str) -> None:
    ...
    with open(legacy_path_str, 'r', encoding='utf-8') as file_descriptor_obj:
        legacy_state_dict: Dict = json.load(file_descriptor_obj)

    dest_data_dict: Dict = {}
    if os.path.exists(state_file_path_str):
        with open(state_file_path_str, 'r', encoding='utf-8') as file_descriptor_obj:
            dest_data_dict = json.load(file_descriptor_obj)
...
def save_state(state_file_path_str: str, state_dict: Dict[str, Any]) -> None:
    ...
    if os.path.exists(state_file_path_str):
        try:
            with open(state_file_path_str, 'r', encoding='utf-8') as file_descriptor_obj:
                data_dict_dict = json.load(file_descriptor_obj)
        except Exception:
            pass
...
def _load_and_sync_state(paths_dict: Dict[str, str], channel_name_str: str, reset_mode_bool: bool) -> Dict[str, Any]:
    ...
    files_set_set, ids_set_set, max_idx_int, max_bytes_int = scan_results_tuple
    
    # Sincroniza usando conjuntos (Sets) para evitar duplicatas
    processed_files_set: Set[str] = set(state_dict["processed_files"]) | files_set_set
    processed_ids_set: Set[str] = set(state_dict["processed_ids"]) | ids_set_set
...
def _select_files_to_process(channel_path_str: str, all_files_list: List[str], processed_ids_list: List[str], archive_path_str: str) -> List[str]:
    ...
    for vid_id_str, files_list_list in id_map_dict.items():
        ...
        to_process_list_list.append(best_file_str)
        
        # Arquiva os arquivos "reservas" (versões piores do mesmo vídeo)
        if len(sorted_files_list_list) > 1:
            _archive_files(channel_path_str, archive_path_str, sorted_files_list_list[1:])
            
    return to_process_list_list
    ...
    for f_str, block_str, meta_dict_dict in pre_processed_results:
        ...
        if not vol_content_str or (len(vol_content_str) + len(block_str) > MAX_CHARS):
            if vol_content_str and len(vol_content_str) + len(block_str) > MAX_CHARS:
                ...
                _save_volume(paths_dict, channel_name_str, vol_idx_int, vol_content_str, vol_meta_list_list)
                vol_idx_int += 1
        ...
        vol_meta_list_list.append(meta_dict_dict)
...
def _initialize_volume_content(paths_dict: Dict[str, str], channel_name_str: str, vol_idx_int: int, reset_mode_bool: bool) -> str:
    ...
    with open(vol_path_str, 'r', encoding='utf-8') as file_descriptor_obj:
        content_str: str = file_descriptor_obj.read()
...
def _process_single_file(full_path_str: str, filename_str: str, global_meta_dict_dict: Dict[str, Any]) -> Tuple[str, Dict[str, str]]:
    """
    ...
    try:
        ...
        meta_dict_dict: Dict[str, str] = get_metadata(full_path_str)
        ...
        if meta_dict_dict["id"] in global_meta_dict_dict:
            _enrich_metadata(meta_dict_dict, global_meta_dict_dict[meta_dict_dict["id"]])
        elif filename_str in global_meta_dict_dict: # Fallback por nome de arquivo se ID falhar
            _enrich_metadata(meta_dict_dict, global_meta_dict_dict[filename_str])
...
def _enrich_metadata(meta_dict_dict: Dict[str, str], global_video_dict: Dict[str, Any]) -> None:
    """
    ...
    """
    # Só sobrescreve se o dado atual for genérico ("Sem Título", "Desconhecida")
    if meta_dict_dict.get("title") in ("Sem Título", "", None):
        meta_dict_dict["title"] = global_video_dict.get("title") or global_video_dict.get("titulo") or "Sem Título"
        
    if meta_dict_dict.get("date") in ("Desconhecida", "", None):
        # Priorizamos 'publish_date' que é o padrão do history.py, mas aceitamos 'upload_date' como fallback.
        date_val = global_video_dict.get("publish_date") or global_video_dict.get("upload_date") or global_video_dict.get("data")
        if date_val:
            meta_dict_dict["date"] = format_date(date_val)
...
def _load_global_metadata(state_path_str: str) -> Dict[str, Any]:
    ...
    try:
        with open(state_path_str, 'r', encoding='utf-8') as file_descriptor_obj:
            data_dict_dict: Dict = json.load(file_descriptor_obj)
            # O Escriba usa 'video_id' ou 'id' dependendo da versão. Verificamos ambos.
            videos_list = data_dict_dict.get("videos", [])
            metadata_map = {}
            for v_dict in videos_list:
                vid_id = v_dict.get("video_id") or v_dict.get("id")
                if vid_id:
                    metadata_map[vid_id] = v_dict
            return metadata_map
    except Exception:
        return {}


def _save_volume(paths_dict: Dict[str, str], channel_name_str: str, idx_int: int, content_str: str, meta_list_list: List[Dict[str, str]]) -> None:
    """
...
def consolidar_por_canal(base_dir_path_str: str, reset_mode_bool: bool = False) -> None:
    ...
    if files_list or has_archives_bool:
        ...
        process_channel(base_dir_path_str, channel_name_str, reset_mode_bool=reset_mode_bool)
    else:
...
def main() -> None:
    ...
    """
    parser_obj = argparse.ArgumentParser(description="LEXIS: Consolidador de Transcrições")
    parser_obj.add_argument("path", nargs="?", default=".", help="Caminho da pasta a processar")
