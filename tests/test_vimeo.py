...
def test_filter_vimeo_cookies_logic(tmp_path: Path):
    ...
    content_str: str = (
        ...
        "evil.com\tTRUE\t/\tFALSE\t0\tID\tvimeo.com_fake\n"
        "#HttpOnly_.vimeo.com\tTRUE\t/\tFALSE\t0\tSID2\tvalue4\n"
        "#HttpOnly_.other.com\tTRUE\t/\tFALSE\t0\tID2\tvalue5\n"
    )
    ...
    assert ".vimeo.com\tTRUE\t/\tFALSE\t0\tSID\tvalue1\n" in result_str
    assert ".akamaized.net\tTRUE\t/\tFALSE\t0\tGAIA\tvalue2\n" in result_str
    assert "#HttpOnly_.vimeo.com\tTRUE\t/\tFALSE\t0\tSID2\tvalue4\n" in result_str
    assert "other.com" not in result_str
    assert "evil.com" not in result_str
    assert "# Netscape" in result_str

def test_filter_vimeo_cookies_missing_file():
    """Verifica se a função lida graciosamente com a ausência do arquivo de cookies."""
    # Não deve levantar erro
    filter_vimeo_cookies(Path("non_existent_cookies_file.txt"))
