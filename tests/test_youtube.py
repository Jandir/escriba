...
def test_filter_youtube_cookies_logic(tmp_path: Path):
    ...
    content_str: str = (
        ...
        "malicious-youtube.com\tTRUE\t/\tFALSE\t0\tID\tvalue4\n"
        "#HttpOnly_bank.com\tTRUE\t/\tFALSE\t0\tSESSION\tsecret\n"
        "#HttpOnly_.youtube.com\tTRUE\t/\tFALSE\t0\tHSID\tvalue5\n"
    )
    ...
    assert "GAIA\tvalue2" in result_str
    assert "HSID\tvalue5" in result_str
    assert "value3" not in result_str
    assert "malicious-youtube.com" not in result_str
    assert "bank.com" not in result_str
