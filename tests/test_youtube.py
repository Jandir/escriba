...
def test_filter_youtube_cookies_logic(tmp_path: Path):
    ...
    content_str: str = (
        ...
        "evil-youtube.com\tTRUE\t/\tFALSE\t0\tEVIL\tvalue4\n"
        "#HttpOnly_.youtube.com\tTRUE\t/\tFALSE\t0\tHTTPONLY_SID\tvalue5\n"
        "#HttpOnly_.evil-google.com\tTRUE\t/\tFALSE\t0\tEVIL_HTTPONLY\tvalue6\n"
    )
    ...
    # Valida presença das cookies corretas
    assert ".youtube.com\tTRUE" in result_str
    assert ".google.com\tTRUE" in result_str
    assert "#HttpOnly_.youtube.com" in result_str
    assert "# Netscape" in result_str

    # Valida exclusão de cookies que vazam dados (cross-domain)
    assert "other.com" not in result_str
    assert "evil-youtube.com" not in result_str
    assert "evil-google.com" not in result_str
