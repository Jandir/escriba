...
def test_filter_youtube_cookies_logic(tmp_path: Path):
    ...
    content_str: str = (
        ...
        ".notyoutube.com\tTRUE\t/\tFALSE\t0\tSID\tvalue4\n"
        "evilyoutube.com.br\tTRUE\t/\tFALSE\t0\tSID\tvalue5\n"
    ...
    assert "#HttpOnly_.youtube.com\tTRUE" in result_str
    assert "other.com" not in result_str
    assert "notyoutube.com" not in result_str
    assert "evilyoutube.com.br" not in result_str
