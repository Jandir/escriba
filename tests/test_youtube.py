...
def test_filter_youtube_cookies_logic(tmp_path: Path):
    ...
    content_str: str = (
        ...
        ".evil-youtube.com\tTRUE\t/\tFALSE\t0\tID\tvalue4\n"
        "#HttpOnly_.youtube.com\tTRUE\t/\tFALSE\t0\tSID\tvalue5\n"
    )
    ...
    assert "#HttpOnly_.youtube.com\tTRUE\t/\tFALSE\t0\tSID\tvalue5\n" in result_str
    assert "other.com" not in result_str
    assert "evil-youtube.com" not in result_str
    assert "# Netscape HTTP Cookie File" in result_str
