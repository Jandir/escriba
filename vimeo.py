...
def filter_vimeo_cookies(cookies_path_obj: Path) -> None:
    ...
    try:
        ...
        for line_str in lines_list:
            if line_str.startswith("# ") or line_str.strip() == "" or line_str == "#\n":
            ...
            if len(parts) >= 7:
                ...
                if domain.startswith("#HttpOnly_"):
                    actual_domain = domain[10:]
                else:
                    actual_domain = domain

                if actual_domain.endswith('.vimeo.com') or actual_domain == 'vimeo.com' or actual_domain.endswith('.akamaized.net') or actual_domain == 'akamaized.net':
                    filtered_lines_list.append(line_str)
