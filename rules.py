...
# BOLT OPTIMIZATION: Global pre-compiled regexes for NLP hot paths
_MUSIC_NOTES_PATTERN = re.compile(r'[♪♫#]')
...
def clean_asr_artifacts(text_str: str) -> str:
    ...
    cleaned = _MUSIC_NOTES_PATTERN.sub('', text_str)
...
_PUNCT_CAP_PATTERN = re.compile(r'([.!?]\s+)(\w)')
...
def fix_sentence_capitalization(text_str: str) -> str:
    ...
    clean_txt = _PUNCT_CAP_PATTERN.sub(_cap_match, clean_txt)
    ...
    clean_txt = _QUESTION_END_PATTERN.sub(_q_match, clean_txt)
    return clean_txt
