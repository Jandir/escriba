...
## 2026-12-08 - Path instantiation overhead inside loops
**Learning:** Instantiating `Path` objects inside tight loops solely for concatenation with string suffixes or extensions creates unnecessary memory overhead. Double list comprehensions scanning the same list repeatedly to filter by extension further compound iteration overhead. In `escriba.py`, combining multiple list comprehensions into a single loop using `os.path.join` and strings avoided redundant iterations and allocations during disk file checks.
**Action:** When filtering or modifying lists of paths inside tight loops, convert `Path` bases to strings outside the loop, use a single `for` loop with conditional checks instead of multiple list comprehensions, and use `os.path.join` for concatenation instead of the `/` operator with Path objects.
