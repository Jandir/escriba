...
# BOLT OPTIMIZATION:
# Pre-compiled regex patterns for `extract_video_id` to avoid on-the-fly compilation overhead
# during frequent parsing of filenames.
...
def extract_video_id(filename_or_path: str) -> str:
    ...
    Passos do Algoritmo:
    1. Usamos `os.path.basename(filename_or_path)` para descartar toda a estrutura de pastas do sistema
    ...
    """
    # BOLT OPTIMIZATION:
    # Use os.path.basename instead of Path().name to avoid Path object instantiation overhead.
    ...
    if matches:
        ...
        for m in reversed(matches):
            ...
            if pos + 11 == len(name) or not _YT_ID_CHAR_PATTERN.match(name[pos+11]):
                return m
        return matches[-1]
