...
# BOLT OPTIMIZATION: Regex compilation is expensive. We pre-compile all regex patterns
# globally to avoid parsing and compiling them every time extract_video_id is called.
_EXT_PATTERN = re.compile(r'\.(srt|md|txt|json|vtt)$', flags=re.IGNORECASE)
_ORIG_PATTERN = re.compile(r'-orig$', flags=re.IGNORECASE)
_LANG_PATTERN = re.compile(r'[.\-][a-z]{2}(?:-[a-z]{2,3})?$', flags=re.IGNORECASE)
_ID_MATCH_PATTERN = re.compile(r'^[A-Za-z0-9_-]{11}$')
_ID_FIND_PATTERN = re.compile(r'([A-Za-z0-9_-]{11})')
_ID_ADJACENT_PATTERN = re.compile(r'[A-Za-z0-9_-]')
...
def extract_video_id(filename_or_path: str) -> str:
    ...
    """
    # BOLT OPTIMIZATION: os.path.basename is ~3x faster than instantiating Path().name for string parsing
    ...
    if matches:
        ...
        for m in reversed(matches):
            ...
            if pos + 11 == len(name) or not _ID_ADJACENT_PATTERN.match(name[pos+11]):
                return m
        return matches[-1]
